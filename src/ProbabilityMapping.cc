...
//        std::cout<<"semi dense mapping took: "<< duration << "s" << std::endl;
    ...
    std::cout<<"semi dense mapping took: "<< duration << "s" << std::endl;
#endif
    std::string strFileName("./data/semi_pointcloud.txt");
    std::ofstream fileOut(strFileName.c_str(), std::ofstream::out);
    if(!fileOut){
...
//                if (kf->depth_map_.at<float>(y,x) > 0.000001) {
                if (kf->depth_map_checked_.at<float>(y,x) > 0.000001) {
                    ...
                    fileOut << std::to_string(Pw[0]) + " " + std::to_string(Pw[1]) + " " + std::to_string(Pw[2]) + " "
                            + std::to_string(vr) + " " << std::to_string(vg) + " " + std::to_string(vb) << std::endl;
                    ///////////////////////////////
                    p.x = (float)Pw[0];  p.y = (float)Pw[1];  p.z = (float)Pw[2];
                    p.r = (vr * 255.0 );
                    p.g = (vg * 255.0);
                    p.b = (vb * 255.0);
    ...
    pcl::io::savePCDFileASCII("./data/semi_pointcloud-pcl-1-8.pcd", cloud);
...
void ProbabilityMapping::SemiDenseLoop(){
    ...
    for(size_t i =0;i < vpKFs.size(); i++ )
    {
        ...
        double duration;
        clock_gettime(CLOCK_MONOTONIC, &start);
        cout<<"semidense_Info:    vpKFs.size()--> "<< vpKFs.size()<< std::flush << std::endl;
